    ...
    <main class="modern-main">
        <section id="filters" class="card mb-4">
            <form action="all_tournaments.php" method="GET" class="row g-3 align-items-center modern-form">
                <div class="col-md-4">
                    <input type="text" name="name" class="form-control" placeholder="Filtra per nome..."
                        value="<?php echo htmlspecialchars($search_name); ?>">
                </div>
                <div class="col-md-4">
                    <select name="filter" class="form-select">
                        <option value="all" <?php if ($filter_type === 'all')
                            echo 'selected'; ?>>Tutti i tornei</option>
                        <option value="in_progress_all" <?php if ($filter_type === 'in_progress_all')
                            echo 'selected'; ?>>
                            Tornei in corso (tutti)</option>
                        <option value="in_progress_participating" <?php if ($filter_type === 'in_progress_participating')
                            echo 'selected'; ?>>Tornei in corso (a cui partecipo)</option>
                        <option value="completed_participating" <?php if ($filter_type === 'completed_participating')
                            echo 'selected'; ?>>Tornei completati (a cui ho partecipato)</option>
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <ol>
                        <?php foreach ($top_players as $index => $player): ?>
                            <li class="player-list-item">
                                ...
                                <div>
                                    <a
                                        href="/views/view_profile.php?uid=<?php echo $player['userId']; ?>"><?php echo htmlspecialchars($user_map[$player['userId']] ?? 'Sconosciuto'); ?></a>
                                    <?php if (!empty($player['decklist_name'])): ?>
                                        - <a
                                            href="/views/view_decklist.php?tid=<?php echo $tournament['id']; ?>&uid=<?php echo $player['userId']; ?>"><?php echo htmlspecialchars($player['decklist_name']); ?></a>
                                    <?php endif; ?>
                                </div>
                            </li>
                        <?php endforeach; ?>
            ...
            <ul class="pagination justify-content-center">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <li class="page-item <?php if ($i == $page)
                        echo 'active'; ?>">
                        <a class="page-link"
                            href="?page=<?php echo $i; ?>&name=<?php echo urlencode($search_name); ?>&filter=<?php echo urlencode($filter_type); ?>"><?php echo $i; ?></a>
                    </li>
                <?php endfor; ?>
            </ul>
        </nav>
